...
def sort_dict_by_date_border_measure_value(list_of_dicts):
    """
...
def reverse_sort_dict_by_date_border_measure_value(list_of_dicts):
    """
    args: list_of_dicts is the input data table, ascending is a bool
    """
    from operator import itemgetter
    sorted_list_of_dicts = sorted(list_of_dicts, key=itemgetter('Date', 'Value', 'Measure', 'Border'), reverse=True)
    # print('\n\nsorted_list_of_dicts\n', ascending, '\n', sorted_list_of_dicts)
    return sorted_list_of_dicts
...
def parse_subsets_with_same_value(input_table, input_key):
    ...
    for i, row in enumerate(input_table):
        if i == 0:
            ...
            input_value_set.add(row[input_key])
            ...
            if i == (table_length - 1): #final row in table
                ...
                start_slice_index_list.append(i + 1)
            ...
            if i == (table_length - 1): #final row in table
                ...
                input_value_set.add(input_table[i][input_key])
        # print('row',row)
        # print('input_value_set', input_value_set)

    input_value_list = sorted(list(input_value_set)) #change set back into list
    # print('\nnumber of items in list = ', len(start_slice_index_list))
    # print('\ntable_subset_list\n', table_subset_list)
    # print('\ninput_value_list = \n', input_value_list)
    # print('\nstart_slice_index_list\n', start_slice_index_list)
    ...
    output_table = []
    for i in range(0, len(start_slice_index_list)-1):
        output_table.append(input_table[start_slice_index_list[i] : start_slice_index_list[i+1]]) 
    return output_table, input_value_list

def quick_prase(input_table):
    """
    Given an ascending-sorted input_table and a key (from a key:value pair), returns a lists of subsets of the input_table where all rows have the same value for a given key.
    Parameters:
    input_table: type is list of dictionaries.
    ...
    # unit check: sort input_table by key to ensure that only only ascending sorted tables are used
    from operator import itemgetter
    input_table = sorted(input_table, key=itemgetter('Date', 'Border', 'Measure'))
    #Parse_subsets_with_same_value using the unit-checked input to get start and end slice indexes
    table_length = len(input_table)
    for i, row in enumerate(input_table):
        if i == 0:
...
def reduce_similar_values_and_calclate_averages(ascending_sorted_table):
    ...
    for subset_by_dates_border in ascending_sorted_table:
        for subset_by_date_border_measure in subset_by_dates_border:
            if subset_by_date_border_measure == []:
                break
            if type(subset_by_date_border_measure) != type(list([])):
                break
            temp_values_list = []
            temp_row = {}
            print('subset_by_date_border_measure', subset_by_date_border_measure)
            for subset_by_date_border_and_measure in subset_by_date_border_measure:
                temp_values_list.append(int(subset_by_date_border_and_measure['Value']))
                temp_row.clear()
                temp_row.update(subset_by_date_border_and_measure)
            ...
            # print(len(temp_values_list))
            print('temp_values_list', temp_values_list)
            temp_value = functools.reduce(lambda acc, x: acc + x, temp_values_list)
            # print(temp_value)
...
def round_values(list_of_dictionraries, key_name):
    for dictionary in list_of_dictionraries:
        new_value = round(dictionary[key_name])
        dictionary.update({key_name : new_value})
    return list_of_dictionraries


data_entries1 = import_csv_with_dictreader(input)

print('\n\ndata_entries1\n', data_entries1) 
print('len(data_entries1)\n', len(data_entries1)) 
...
# print('\n\ndata_entries2\n', data_entries2)
# print('len(data_entries2)', len(data_entries2))

#print(data_entries1)

data_entries3 = sort_dict_by_date_border_measure_value(data_entries2)
# print('len(data_entries3)', len(data_entries3))
#print('\n\ndata_entries3\n', data_entries3)
del data_entries2
...
print('\nTest shape of table_parsed_by_date')
print('len(table_parsed_by_date)', len(table_parsed_by_date))
for i, entries_with_same_date in enumerate(table_parsed_by_date):
    ...
    print('date_values_list', date_values_list)
print('table_parsed_by_date', table_parsed_by_date)
# *A is ok
...
for i, subset_with_same_date in enumerate(table_parsed_by_date_and_border):
    ...
    for j, entries_parsed_by_date_and_border in enumerate(subset_with_same_date):
        ...
        print('entries_parsed_by_date_and_border', entries_parsed_by_date_and_border)
print(table_parsed_by_date_and_border)

print('\n*C* table_parsed_by_date_border_and_measure')
table_parsed_by_date_border_and_measure = []
measure_values_list = []
for i, subset_with_same_date in enumerate(table_parsed_by_date_and_border):
    if type(subset_with_same_date) == type(dict([])):
        print('true)')
...
print('table_parsed_by_date_border_and_measure', table_parsed_by_date_border_and_measure)
# print('\n')
...
list_of_fully_parsed_dicts = []
fully_parsed = []
for i, subset_with_same_date in enumerate(data_entries4):
    if type(subset_with_same_date) == type(dict([])):
        list_of_fully_parsed_dicts.append(subset_with_same_date)
    for j, subset_with_same_date_and_same_border in enumerate(subset_with_same_date):
        if type(subset_with_same_date) == type(dict([])):
            list_of_fully_parsed_dicts.append(subset_with_same_date_and_same_border)
        for k, subset_with_same_date_border_and_measure in enumerate(subset_with_same_date_and_same_border):
            if type(subset_with_same_date_border_and_measure) == type(dict([])):
                list_of_fully_parsed_dicts.append(subset_with_same_date_border_and_measure)
            for l, fully_parsed_subset in enumerate(subset_with_same_date_border_and_measure):
                if type(fully_parsed_subset) == type(dict([])):
                    list_of_fully_parsed_dicts.append(fully_parsed_subset)
# Test shape of list_of_fully_parsed_dicts
print('\nTest shape of list_of_fully_parsed_dicts')
for i, subset_with_same_date in enumerate(table_parsed_by_date):
    print('len(subset_with_same_date) in table_parsed_by_date_and_border [', i, ']', len(subset_with_same_date))
    print('len(subset_with_same_date) in table_parsed_by_date_and_border [', i, ']', len(subset_with_same_date))     
    for j, subset_with_same_date_and_same_border in enumerate(subset_with_same_date):
        ...
        # print(value_list)
        # print('\t\tlen(subset_with_same_date_border_and_measure) in subset_with_same_date_and_same_border [', i, '][', j, '][', k, ']', len(subset_with_same_date_border_and_measure))
print('list_of_fully_parsed_dicts', list_of_fully_parsed_dicts)
...
reversed_table_after_reducing = reverse_sort_dict_by_date_border_measure_value(table_after_reducing)
# sorted(table_after_reducing, key=itemgetter('Date', 'Value', 'Measure', 'Border'), reverse=False)
del table_after_reducing
rounded_reversed_table_after_reducing = round_values(reversed_table_after_reducing, 'Average')


export_csv_with_dictwriter2(output, rounded_reversed_table_after_reducing)
